...
xbacklight_main (int argc, std::vector<const char*> args )
{
    ...
    backlight_new_ = backlight_reply->atom;
    ...
    if (backlight_new_ == XCB_NONE && backlight_legacy == XCB_NONE) {
	...
	for (int o = 0; o < resources_reply->num_outputs; o++)
	{
		...
		if (prop_reply->range &&
			...
			if (new_ > max) new_ = max;
			if (new_ < min) new_ = min;
			step = (new_ - cur) / steps;
